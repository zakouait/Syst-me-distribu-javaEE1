...
public class C05 {
    public static void main(String[] args) {
        try {
            ...
            DatagramSocket cc = new DatagramSocket(9005);
int cmt=0;
            ...
            while (i == 1) {
                while ( cmt<listeservice.length) {Timer timer= new Timer() ;
                        ...
                        public void run() {
                                ...
                                timer.schedule(new TimerTask() {
                                    ...
                                    public void run() {
                                        ...
                                        DatagramPacket sendm = new DatagramPacket(newToken.getBytes(), newToken.getBytes().length, ip,9006);
                                        try {
                  ...
                  //envoyer toker
                    ...
                    DatagramPacket sendp = new DatagramPacket(sentData, sentData.length, ip, 9006);
                    c.send(sendp);cmt++;
                }
                //recevoir token
                DatagramPacket receivep = new DatagramPacket(receiveData, receiveData.length);
